...
const App = () => {
  ...
  const submitHandler = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const hasErrorMsg =
      ...
      Object.values(errors).every(
        (value) => value == "" && tempColors.length > 0
      );
    if (!hasErrorMsg) {
      setErrors(errors);
      return;
  ...
  return (
    <main className="container mx-auto xl:px-20 flex flex-col gap-4">
      <Button className="bg-indigo-600 mx-auto" onClick={openModal}>
        Add New Product
      </Button>
      {/* Add Product Modal */}
      <Modal isOpen={isOpen} close={closeModal} title={"Add New Product"}>
        <form className="flex flex-col space-y-4" onSubmit={submitHandler}>
          ...
          <div className="flex gap-4">
            ...
            <Button
              className="bg-red-500 transition-colors duration-200 ease-in-out"
              onClick={onCancel}
            >
              Cancel
            </Button>
